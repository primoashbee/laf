...
class UploadController extends Controller
{
    ...
    public function printed(Request $request){
        ...
        if(auth()->user()->is_admin){
            ...
            $clients = Client::where('received',true)->where('branch',$branch)->get();
        }
    ...
    public function byBatch(Request $request, $batch_id){
        ...
        if(auth()->user()->is_admin){
            if($request->has('branch')){
                $clients = Client::where('batch_id',$batch_id)->where('branch',$request->branch)->get();
            }
        }else{
            if($request->has('branch')){    
                if($request->branch != $branch){
            ...
            $clients = Client::where('batch_id',$batch_id)->where('branch',$branch)->get();
        }   
    ...
    public function unprinted(Request $request){
        ...
        if(auth()->user()->is_admin){
            if($request->has('branch')){
                $clients = Client::where('received',false)->where('branch',$request->branch)->get();
            }
            ...
            $clients = Client::where('received',false)->where('branch',$branch)->get();
        }
    ...
    public function printList($clients){
        ...
        foreach ($clients as $client) {
            ...
            if ($spouse_name) {
                ...
                $spouse_total_income =  $spouse_other_income_monthly_estimated_earnings +
                                        $monthly_gross_income_at_work+
                                        $monthly_income_for_spouse_business;

                dd($monthly_income_for_spouse_business);
                ...
                $total_household_income= $this->currency(
